  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-ink-950">
      <div className="flex items-center justify-between px-4 py-3 safe-top">
        <span className="font-display text-section-title text-white">Scan barcode</span>
        <button onClick={onClose} className="flex items-center justify-center rounded-control p-2 text-white/80 hover:bg-white/10" aria-label="Close">
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        {status === 'scanning' && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div className="h-40 w-64 rounded-panel border-2 border-primary-500" />
          </div>
        )}
      ...
      {torchSupported && status === 'scanning' && (
        <div className="flex justify-center pb-8 pt-4 safe-bottom">
          <button
            onClick={toggleTorch}
            className={`flex items-center gap-2 rounded-pill px-4 py-2.5 text-button ${torchOn ? 'bg-warning-400 text-ink-900' : 'bg-white/10 text-white'}`}
          >
...
function ScannerMessage({ icon, title, body, action }) {
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-8 text-center">
      {icon}
      <p className="font-display text-page-title text-white">{title}</p>
