...
export default function AdminSupportMode() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="rounded-2xl border-2 border-warning-400 bg-warning-50 p-4 text-warning-950 space-y-1 shadow-sm">
      ...
      <div className="rounded-panel border border-line bg-surface p-5 bg-white space-y-3">
        <div className="flex items-center justify-between border-b border-ink-100 pb-3">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-xl bg-success-600 text-white flex items-center justify-center font-black">
          ...
          <span className="badge bg-success-100 text-success-800 font-bold">
        ...
        <div className="grid grid-cols-5 gap-2 text-xs font-bold">
          <button
            ...
            className={`py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 border ${activeTab === 'counter' ? 'bg-success-50 border-success-500 text-success-800' : 'border-ink-200 text-ink-600'}`}
          >
          ...
          <button
            ...
            className={`py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 border ${activeTab === 'dashboard' ? 'bg-success-50 border-success-500 text-success-800' : 'border-ink-200 text-ink-600'}`}
          >
          ...
          <button
            ...
            className={`py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 border ${activeTab === 'inventory' ? 'bg-success-50 border-success-500 text-success-800' : 'border-ink-200 text-ink-600'}`}
          >
          ...
          <button
            ...
            className={`py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 border ${activeTab === 'customers' ? 'bg-success-50 border-success-500 text-success-800' : 'border-ink-200 text-ink-600'}`}
          >
          ...
          <button
            ...
            className={`py-2 px-2 rounded-xl flex items-center justify-center gap-1.5 border ${activeTab === 'closeday' ? 'bg-success-50 border-success-500 text-success-800' : 'border-ink-200 text-ink-600'}`}
          >
      ...
      {activeTab === 'dashboard' && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-panel border border-line bg-surface p-4 bg-white">
            <span className="text-xs font-semibold uppercase text-ink-400">Total Sales</span>
            <p className="text-xl font-bold text-success-700 mt-1">{formatKES(metrics.totalSalesRevenue)}</p>
          </div>
          <div className="rounded-panel border border-line bg-surface p-4 bg-white">
            <span className="text-xs font-semibold uppercase text-ink-400">Gross Margin</span>
            <p className="text-xl font-bold text-success-700 mt-1">{formatKES(metrics.totalGrossProfit)}</p>
          </div>
          <div className="rounded-panel border border-line bg-surface p-4 bg-white">
            <span className="text-xs font-semibold uppercase text-ink-400">Uncollected Debt</span>
            <p className="text-xl font-bold text-danger-600 mt-1">{formatKES(metrics.totalOutstandingDebt)}</p>
          </div>
          <div className="rounded-panel border border-line bg-surface p-4 bg-white">
      ...
      {activeTab === 'counter' && (
        <div className="rounded-panel border border-line bg-surface p-6 bg-white text-center space-y-3">
      ...
      {activeTab === 'inventory' && (
        <div className="rounded-panel border border-line bg-surface p-6 bg-white text-center space-y-2">
          <Boxes className="h-10 w-10 mx-auto text-ink-300" />
          <h3 className="font-bold text-ink-900">Inventory Status</h3>
          <p className="text-xs text-ink-500">
            Total Capital Deployed: <strong className="text-ink-900">{formatKES(metrics.totalInventoryCost)}</strong> &middot; Out of Stock: <strong className="text-danger-600">{metrics.outOfStockCount}</strong>
          </p>
        </div>
      )}
      ...
      {activeTab === 'customers' && (
        <div className="rounded-panel border border-line bg-surface p-6 bg-white text-center space-y-2">
          ...
          <p className="text-xs text-ink-500">
            Registered Customers: <strong className="text-ink-900">{metrics.customersCount}</strong> &middot; Outstanding Market Exposure: <strong className="text-danger-600">{formatKES(metrics.totalOutstandingDebt)}</strong>
          </p>
        </div>
      )}
      ...
      {activeTab === 'closeday' && (
        <div className="rounded-panel border border-line bg-surface p-6 bg-white text-center space-y-2">
          ...
          <p className="text-xs text-ink-500">
            Cash Sales: <strong className="text-success-700">{formatKES(metrics.cashSalesAmount)}</strong> &middot; M-Pesa Sales: <strong className="text-success-700">{formatKES(metrics.mpesaSalesAmount)}</strong>
          </p>
        </div>
      )}
    </div>
